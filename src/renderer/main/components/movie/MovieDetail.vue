...
export default defineComponent({
    ...
    setup: () => {
        ...
        const edits = ref<any[]>([])
        const libInfo = computed(() => store.state.Libs.libs.find((lib: any) => lib.id === movieInfo.value.media_lib_id) || {})

        async function freshMovieInfo() {
            if (route.params.movieId) {
                movieInfo.value = await store.dispatch("getMovieInfo", route.params.movieId)
        ...
        let main_actors = computed(() => actors.value.filter((v, index) => v && index < 4))
