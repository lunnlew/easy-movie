    ...
    <el-form ref="form" :model="libform" label-width="0px">
        ...
        <el-input v-model="libform.name_cn" placeholder="请输入关键字">
          <template #prepend>
            <el-select
              ...
              style="width: 130px"
            >
              <el-option label="themoviedb" value="themoviedb"></el-option>
            </el-select>
          </template>
          <template #append>
      ...
      <el-form-item style="text-align: right; margin-top: 20px">
        <el-button
          size="medium"
          type="primary"
          :disabled="!currentRow.title"
          @click="onSubmit"
        >
          <span v-if="currentRow.title"
            >已选择({{ currentRow.year || "年份未知" }})</span
          >
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    const scraper_id = ref("themoviedb");
    return {
      onSubmit,
      scraper_id,
      libform,
